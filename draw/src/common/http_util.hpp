...
namespace HTTPUtil{
    namespace StatusCodeHandle {
        namespace {
            inline const constexpr int _INT_MIN{ -2147483648 };
        }
        enum status {
        ...
        const inline json getJsonResult(status state, std::string_view msg = "", const json& extra = json(), int status_code = _INT_MIN) {
        ...
        const inline json getJsonResult(status state, const int status_code,const json& extra) {
            return getJsonResult(state, "", extra, status_code);
        }
        ...
        const inline json getSimpleJsonResult(int code, std::string_view msg = "",const json& extra = json(), int status_code = _INT_MIN) {
            return getJsonResult(getStatus(code), msg, extra, status_code);
        };
